...
app.get("/api/v1/products", (req, res) => {
  ...
  db.all("SELECT * FROM t_prodotti",  (err, results) => {
    if (err) {
      console.error(err);
...
app.get("/api/v1/products/:id", (req, res) => {
  ...
  db.get("SELECT * FROM t_prodotti WHERE id =?", [id], (err, results) => {
    if (err) {
      console.error(err);
      res.status(500).send("Errore nel database");
    } else if (!results) {
...
app.post("/api/v1/products/", (req, res) => {
  ...
  db.run("INSERT INTO t_prodotti ( nome ) VALUES (?)", [nome], (err, results) => {
    if (err) {
      console.error(err);
      res.status(500).send("Errore nel database");
    } else {
      // la post restituisce l'oggetto con l'id inserito su database
      res.status(201).json({ nomeInput: nome });
    }
  });
});
/*
